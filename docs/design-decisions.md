...
Key architectural choices in The Url Mentalist and the reasoning behind them.
...
**Why:** The core bottleneck is I/O â€” DNS lookups, HTTP fetches, TLS handshakes, WHOIS queries. Go's goroutines make it trivial to run all 18 checks concurrently with a `sync.WaitGroup`. The result is 300â€“700ms median scan time with no async/await complexity and clean per-goroutine panic recovery. A Python or Node implementation would need explicit async machinery for the same concurrency and would carry more runtime overhead per request.
...
**Why:** Explainability is a first-class goal. Every verdict can be traced to specific, named signals with human-readable reasons. An ML model would give accurate verdicts but opaque ones â€” users and integrators couldn't understand or trust why a URL was flagged. The tradeoff is accepted: The Url Mentalist will miss sophisticated phishing that mimics legitimate site structure, but it will never produce a verdict nobody can explain.
...
**Why:** Simplicity. The longest checks (WHOIS ~1â€“2s, screenshot ~10â€“30s) dominate latency regardless of ordering. Running fast structural checks first and gating slow network checks on their results would add coordination complexity for minimal gain â€” the total wall time is still bounded by the slowest check.
...
**Why:** The primary access pattern is `url â†’ result` â€” a pure key-value lookup. Valkey handles this in sub-millisecond with built-in LRU eviction and TTL. PostgreSQL would add connection overhead, schema complexity, and index maintenance for a use case that maps directly to a key-value store's strengths.
...
**Why:** `slog` (Go 1.21+) provides structured, leveled logging with no external dependency. The centralized wrapper (`internal/logger`) lets the format be changed in one place â€” colored text in DEV, JSON in production â€” without touching any call site. Full URLs are kept out of logs (scheme+host only) to prevent token leakage in log files.

---

## AGPL-3.0 license

**Decision:** The Url Mentalist is licensed under AGPL-3.0 with a separate commercial license option.
