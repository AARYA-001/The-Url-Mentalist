...
func CheckHTTPCombined(rawURL string) (CombinedHTTPResult, error) {
	...
	// Create HTTP client with robust timeouts and redirect tracking
	transport := &http.Transport{
		TLSClientConfig:       &tls.Config{InsecureSkipVerify: true},
		ResponseHeaderTimeout: 3 * time.Second, // Max time to wait for response headers per request
	...
	client := &http.Client{
		Timeout:   8 * time.Second, // Overall timeout
		Transport: transport,
