...
func loadSecret() []byte {
	loadOnce.Do(func() {
		...
		if s == "" {
			if strings.EqualFold(os.Getenv("ENV"), "PROD") || strings.EqualFold(os.Getenv("ENV"), "PRODUCTION") {
				panic("admintoken: ADMIN_JWT_SECRET is required in production")
			}
			// In development/test mode, generate a secure ephemeral key
			buf := make([]byte, 32)
			if _, err := rand.Read(buf); err != nil {
				panic("admintoken: failed to generate dev secret: " + err.Error())
			}
			secret = buf
			log.Println("admintoken: ADMIN_JWT_SECRET not set; generated ephemeral secret for development session (" + hex.EncodeToString(buf[:8]) + "...)")
			return
		}
		secret = []byte(s)
	})
